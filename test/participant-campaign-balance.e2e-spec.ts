...
import { TransactionCode, TransactionCodeStatus, TransactionType } from '../src/resources/participant-campaign-balance/entities/transaction-code.entity';
import { DataSource, Repository } from 'typeorm';
import { HashService } from '../src/common/hash/hash.service';
import { nanoid } from 'nanoid';
import { CampaignType, AudienceType } from '../src/resources/campaign/entities/campaign-enums';
...
describe('ParticipantCampaignBalanceController (e2e)', () => {
  ...
  beforeAll(async () => {
    ...
    businessCampaignRepository = moduleFixture.get<Repository<BusinessCampaign>>(getRepositoryToken(BusinessCampaign));
    transactionCodeRepository = moduleFixture.get<Repository<TransactionCode>>(getRepositoryToken(TransactionCode));
    await app.init();
  ...
  afterAll(async () => {
    ...
    try {
        await queryRunner.query('TRUNCATE TABLE "transaction_codes" RESTART IDENTITY CASCADE');
        await queryRunner.query('TRUNCATE TABLE "business_campaigns" RESTART IDENTITY CASCADE');
  ...
  it('/participant-campaign-balance/claim-code (POST) - should successfully claim a code', async () => {
    ...
    expect(response.body).toHaveProperty('message', 'Points awarded successfully');
  });
});
