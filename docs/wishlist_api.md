...
This endpoint provides businesses with anonymized, aggregated data from user wishlists. It is designed to give insights into what items customers are interested in, allowing businesses to create targeted campaigns. The data only includes items from users who have given marketing consent.
...
### Response Data (`WishlistAggregate[]`)

The endpoint returns an array of `WishlistAggregate` objects. Each object represents a unique combination of an item name and a category, and includes the total audience size and a list of target dates.
