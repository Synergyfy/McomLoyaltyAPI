...
describe('StampService', () => {
  ...
  beforeEach(async () => {
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        StampService,
        { provide: getRepositoryToken(StampRewardTemplate), useValue: mockRepo },
        { provide: getRepositoryToken(BusinessStampReward), useValue: mockRepo },
  ...
  describe('addStamp', () => {
    it('should add a stamp when triggered', async () => {
      ...
      const reward = {
          id: 'r1',
          is_active: true,
          template: { trigger_method: StampTriggerMethod.PURCHASE, required_stamps: 5, is_hybrid: false }
      };
      ...
      const mockManager = {
          ...
          save: jest.fn().mockImplementation((entity, dto) => Promise.resolve(dto || entity)),
          increment: jest.fn(),
      };
      mockDataSource.transaction.mockImplementation((cb) => cb(mockManager));

      const result = await service.addStamp('b1', 'p1', StampTriggerMethod.PURCHASE);
