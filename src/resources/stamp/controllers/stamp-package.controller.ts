...
export class StampPackageController {
    ...
    findAll(@Query() pagination: PaginationDto) {
        return this.stampPackageService.findAll(pagination.page || 1, pagination.limit || 10);
    }

    @Get('available')
    @Roles(Role.Business)
    @ApiOperation({ summary: 'Business: Get available stamp packages for my tier' })
    getAvailable(@CurrentUser() user: any, @Query() pagination: PaginationDto) {
        ...
        return this.stampPackageService.getAvailablePackages(user.tierId, pagination.page || 1, pagination.limit || 10);
    }
    ...
    confirmPurchase(
        ...
        return this.stampPackageService.confirmPurchase(user.id, transactionId, provider);
    }
    ...
    update(@Param('id', ParseUUIDPipe) id: string, @Body() dto: UpdateStampPackageDto) {
