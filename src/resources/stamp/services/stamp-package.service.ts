...
import { BusinessStampPackage, BusinessStampPackageStatus } from '../entities/business-stamp-package.entity';
...
export class StampPackageService {
    ...
    async findAll(page: number, limit: number): Promise<PaginationResult<StampPackage>> {
    ...
    async remove(id: string): Promise<void> {
        ...
        if (result.affected === 0) throw new NotFoundException('Stamp package not found');
    }

    async getAvailablePackages(tierId: string, page: number, limit: number): Promise<PaginationResult<StampPackage>> {
    ...
    async confirmPurchase(businessId: string, transactionId: string, provider: string) {
    ...
    async deductStamps(businessId: string, stamps: number, manager: any) {
        ...
        if (stampsToDeduct > 0) {
            throw new BadRequestException(`Insufficient stamps. You need ${stampsToDeduct} more stamps from packages.`);
        }
    }
    ...
    async getAggregateBalance(businessId: string): Promise<{ total_balance: number }> {
        ...
        const total_balance = packages.reduce((sum, pkg) => sum + pkg.remaining_stamps, 0);
