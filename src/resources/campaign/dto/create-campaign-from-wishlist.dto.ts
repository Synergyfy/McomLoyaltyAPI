...
export class CreateCampaignFromWishlistDto extends BaseCampaignDto {
    @ApiProperty({
        description: 'The ID of the wishlist aggregate to create the campaign from.',
    })
    @IsUUID()
    wishlistAggregateId: string;

    @ApiProperty({
        description: 'The IDs of the business rewards attached to the campaign (for Business).',
        type: [String],
        required: false,
    })
    @IsOptional()
