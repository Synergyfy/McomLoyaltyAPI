...
describe('CampaignService - Tier Validation', () => {
    ...
    const mockCampaignRepository = {
        ...
        save: jest.fn().mockImplementation((campaign) => Promise.resolve({ id: 'campaign-id', ...campaign })),
    ...
    describe('create (Admin)', () => {
        ...
        it('should create campaign successfully when rewards are within tier limit', async () => {
            ...
            mockRewardRepository.findBy.mockResolvedValue([{ id: 'r1' }, { id: 'r2' }]);

            const result = await service.create(createDto, adminUser);

            expect(mockTierRepository.findOneBy).toHaveBeenCalledWith({ id: 'tier-id' });
        ...
        it('should throw BadRequestException when rewards exceed tier limit', async () => {
            ...
            await expect(service.create(createDto, adminUser)).rejects.toThrow(BadRequestException);
        });
