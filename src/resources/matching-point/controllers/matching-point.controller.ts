...
import { ApiTags, ApiOperation, ApiBearerAuth, ApiResponse, ApiBody, ApiForbiddenResponse, ApiUnauthorizedResponse } from '@nestjs/swagger';
import { MatchingPointService } from '../services/matching-point.service';
import { MatchingPointActivityType, MatchingPointConfig } from '../entities/matching-point-config.entity';
...
export class MatchingPointController {
    ...
    @Get('config')
    @Roles(Role.Admin)
    @ApiOperation({ summary: 'Get all matching point configurations (Admin only)' })
    @ApiResponse({
        status: 200,
    ...
    @Post('adjust')
    @Roles(Role.Admin)
    @ApiOperation({ summary: 'Manually adjust matching points for a business (Admin only)' })
    ...
    async manualAdjustment(@Body() dto: ManualAdjustmentDto) {
        await this.matchingPointService.manualAdjustment(dto.businessId, dto.points, dto.description);
    ...
    @ApiResponse({
        ...
        schema: {
            type: 'object',
            properties: {
                data: { type: 'array', items: { $ref: '#/components/schemas/MatchingPointHistory' } },
                total: { type: 'number' },
                page: { type: 'number' },
                limit: { type: 'number' },
