...
export class CapabilitiesGuard implements CanActivate {
    ...
    async canActivate(context: ExecutionContext): Promise<boolean> {
        const metadata = this.reflector.getAllAndOverride<{ action: ActionType; context: PermissionContext }>(CHECK_PERMISSION_KEY, [
            context.getHandler(),
            context.getClass(),
        ]);
    ...
    private extractContext(action: ActionType, request: any): any {
        ...
        switch (action) {
            ...
            case ActionType.UPDATE_CAMPAIGN:
                return {
                    rewardCount: body.business_reward_ids ? body.business_reward_ids.length : 0,
                };
