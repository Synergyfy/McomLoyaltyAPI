...
export class SetupService {
    ...
    async getBusinessSetupStatus(userId: string, role: Role) {
        ...
        const [hasReward, hasCampaign, hasStaff] = await Promise.all([
            this.businessRewardRepository.count({ where: { business: { id: businessId } } }),
            this.businessCampaignRepository.count({ where: { business: { id: businessId } } }),
    ...
    async getAdminSetupStatus() {
        const [hasTiers, hasRewards, hasCampaigns, hasPointPackages] = await Promise.all([
