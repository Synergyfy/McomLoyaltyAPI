...
describe('PointEarningService', () => {
  ...
  beforeEach(async () => {
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        ...
        { provide: getRepositoryToken(ParticipantCampaignBalance), useValue: mockParticipantCampaignBalanceRepository },
  ...
  describe('awardPoints', () => {
    ...
    const participant = { id: 'participant-id', global_total_points: 0 } as Participant;
    ...
    it('should throw BadRequestException if regular points threshold is reached', async () => {
      ...
      const campaignAtThreshold = { ...businessCampaign, total_points_earned: 100 };
      const mockManager = {
        findOne: jest.fn().mockImplementation((entity) => {
          if (entity === Participant) return participant;
      ...
      await expect(
        service.awardPoints('staff-id', 'Staff', 'participant-id', 'campaign-id', 10),
