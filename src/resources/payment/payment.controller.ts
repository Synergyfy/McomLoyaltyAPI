import { Controller, Post, Body, UseGuards, Headers, Req, Inject, forwardRef } from '@nestjs/common';
...
export class PaymentController {
  ...
  async verifyStripePayment(@Body() verifyPaymentDto: VerifyPaymentDto, @CurrentUser() user) {
    const result = await this.paymentService.verifyStripePayment(verifyPaymentDto, user);
  ...
  async verifyPaypalPayment(@Body() verifyPaymentDto: VerifyPaymentDto, @CurrentUser() user) {
    const result = await this.paymentService.verifyPaypalPayment(verifyPaymentDto, user);
  ...
  async verifyPaypalSubscription(@Body() verifySubscriptionDto: VerifySubscriptionDto, @CurrentUser() user) {
    const result = await this.paymentService.verifyPaypalSubscription(verifySubscriptionDto, user);
  ...
  async subscribe(@Body() subscribeDto: SubscribeDto, @CurrentUser() business: Business) {
    ...
    if (result.status === 'Trial started' || result.status === 'Subscription successful') {
