...
import { PaymentProvider, PaymentStatus, PurchaseType } from '../entities/payment-history.entity';


export class PaymentHistoryQueryDto extends PaginationDto {
    @ApiProperty({ required: false, description: 'Search by transaction ID, business name, or email' })
    @IsOptional()
    @IsString()
    search?: string;
