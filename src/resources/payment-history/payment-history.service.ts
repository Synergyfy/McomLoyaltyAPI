...
export class PaymentHistoryService {
  ...
  async findAll(query: PaymentHistoryQueryDto) {
    ...
    if (payment_provider) {
      queryBuilder.andWhere('paymentHistory.payment_provider = :payment_provider', { payment_provider });
    }

    if (user_type) {
      queryBuilder.andWhere('paymentHistory.user_type = :user_type', { user_type });
    }

    if (purchase_type) {
      queryBuilder.andWhere('paymentHistory.purchaseType = :purchase_type', { purchase_type });
    }

    if (min_amount) {
      queryBuilder.andWhere('paymentHistory.amount >= :min_amount', { min_amount });
    }

    if (max_amount) {
      queryBuilder.andWhere('paymentHistory.amount <= :max_amount', { max_amount });
    }
