...
export class CreateTrainingGuideDto {
    ...
    @ApiProperty({ example: "A comprehensive guide for businesses to master loyalty." })
    @IsString()
    description: string;
    ...
    @ApiPropertyOptional({ type: [String], description: "IDs of Training Videos" })
    @IsArray()
    @IsUUID("4", { each: true })
    @IsOptional()
    video_ids?: string[];

    @ApiPropertyOptional({ type: [String], description: "IDs of Help Center Articles" })
    @IsArray()
    @IsUUID("4", { each: true })
    @IsOptional()
    article_ids?: string[];
