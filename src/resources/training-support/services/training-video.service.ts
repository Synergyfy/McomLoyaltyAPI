...
import { CreateTrainingVideoDto, UpdateTrainingVideoDto, FilterTrainingVideoDto } from "../dto/training-video.dto";
...
export class TrainingVideoService {
    ...
    async findAll(filterDto: FilterTrainingVideoDto): Promise<PaginationResult<TrainingVideo>> {
        const { page = 1, limit = 10, target_audience, title } = filterDto;
        const queryBuilder = this.trainingVideoRepository.createQueryBuilder("video")
            .leftJoinAndSelect("video.targetTiers", "tiers");

        if (target_audience) {
            queryBuilder.andWhere("video.target_audience = :target_audience", { target_audience });
        }

        if (title) {
            queryBuilder.andWhere("video.title ILIKE :title", { title: `%${title}%` });
        }
    ...
    async findOne(id: string): Promise<TrainingVideo> {
        ...
        if (!video) throw new NotFoundException(`Training video with ID ${id} not found`);
        return video;
    }

    async update(id: string, updateDto: UpdateTrainingVideoDto): Promise<TrainingVideo> {
        const video = await this.findOne(id);
        const { target_tier_ids, ...rest } = updateDto;

        if (target_tier_ids) {
            video.targetTiers = await this.tierRepository.findBy({ id: In(target_tier_ids) });
        }
