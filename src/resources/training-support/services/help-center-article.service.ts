...
import { CreateHelpCenterArticleDto, UpdateHelpCenterArticleDto, FilterHelpCenterArticleDto } from "../dto/help-center-article.dto";
...
export class HelpCenterArticleService {
    ...
    async create(createDto: CreateHelpCenterArticleDto): Promise<HelpCenterArticle> {
    ...
    async findAll(filterDto: FilterHelpCenterArticleDto): Promise<PaginationResult<HelpCenterArticle>> {
        const { page = 1, limit = 10, target_audience, title, category } = filterDto;
        const queryBuilder = this.articleRepository.createQueryBuilder("article")
            .leftJoinAndSelect("article.targetTiers", "tiers");

        if (target_audience) {
            queryBuilder.andWhere("article.target_audience = :target_audience", { target_audience });
        }
    ...
    async update(id: string, updateDto: UpdateHelpCenterArticleDto): Promise<HelpCenterArticle> {
        const article = await this.findOne(id);
        const { target_tier_ids, ...rest } = updateDto;

        if (target_tier_ids) {
            article.targetTiers = await this.tierRepository.findBy({ id: In(target_tier_ids) });
        }
