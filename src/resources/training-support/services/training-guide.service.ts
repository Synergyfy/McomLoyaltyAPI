...
import { CreateTrainingGuideDto, UpdateTrainingGuideDto, FilterTrainingGuideDto } from "../dto/training-guide.dto";
...
export class TrainingGuideService {
    ...
    async create(createDto: CreateTrainingGuideDto): Promise<TrainingGuide> {
        ...
        const targetTier = await this.tierRepository.findOne({ where: { id: target_tier_id } });
        if (!targetTier) throw new NotFoundException(`Tier with ID ${target_tier_id} not found`);
    ...
    async findAll(filterDto: FilterTrainingGuideDto): Promise<PaginationResult<TrainingGuide>> {
        ...
        if (title) {
            queryBuilder.andWhere("guide.title ILIKE :title", { title: `%${title}%` });
        }

        if (target_tier_id) {
            queryBuilder.andWhere("guide.target_tier_id = :target_tier_id", { target_tier_id });
        }
    ...
    async findOne(id: string): Promise<TrainingGuide> {
        ...
        if (!guide) throw new NotFoundException(`Training guide with ID ${id} not found`);
        return guide;
    }

    async update(id: string, updateDto: UpdateTrainingGuideDto): Promise<TrainingGuide> {
        ...
        if (article_ids) {
            guide.articles = await this.articleRepository.findBy({ id: In(article_ids) });
        }
