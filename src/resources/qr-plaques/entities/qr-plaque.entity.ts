...
export class QrPlaque extends AbstractBaseEntity {
    ...
    @ApiProperty({ description: 'Footer text displayed on the plaque', required: false })
    @Column({ name: 'footer_text', nullable: true })
    footerText: string;

    @ApiProperty({ description: 'URL where the interaction leads', example: 'https://example.com' })
    @Column({ name: 'content_url' })
    contentUrl: string;

    @ApiProperty({ description: 'URL of the generated QR code image', required: false })
    ...
    @ApiProperty({ enum: QrPlaqueStatus, description: 'Current status of the plaque' })
    @Column({
        type: 'enum',
        enum: QrPlaqueStatus,
    ...
    @ApiProperty({ type: () => Partner, description: 'Owner of the plaque (if assigned to self/partner)' })
    @ManyToOne(() => Partner, { nullable: true })
    @JoinColumn({ name: 'assigned_partner_id' })
    assignedPartner: Partner;

    @ApiProperty({ type: () => Business, description: 'Business that owns/created the plaque' })
    @ManyToOne(() => Business, { nullable: true })
    @JoinColumn({ name: 'assigned_business_id' })
    assignedBusiness: Business;

    @ApiProperty({ type: () => Network, description: 'Network contact assigned to the plaque (pending acceptance)' })
