...
export class WalletCronService {
  ...
  async resetMonthlyAllowances() {
    ...
    for (const membership of activeMemberships) {
      ...
      const monthlyBudget = membership.tier.configuration?.quotas?.monthlyRewardBudget || 0;
      ...
      if (wallet) {
          ...
          try {
              ...
              this.logger.error(`Failed to reset wallet for business ${membership.business.id}`, e);
          } finally {
