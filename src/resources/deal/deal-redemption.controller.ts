...
export class DealRedemptionController {
    ...
    @ApiResponse({ status: 400, description: 'Bad Request (e.g. inventory limit).' })
    purchase(@Body() purchaseDealDto: PurchaseDealDto, @CurrentUser() user: User) {
    ...
    @ApiResponse({ status: 400, description: 'Insufficient points or invalid deal.' })
