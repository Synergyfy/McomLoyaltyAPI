...
export class DealRedemptionService {
    ...
    async purchase(purchaseDealDto: PurchaseDealDto, user: User) {
        ...
        try {
            ...
            if (deal.maxQuantity !== null && deal.soldQuantity + quantity > deal.maxQuantity) {
            ...
            if (deal.perCustomerLimit) {
                ...
                if (userRedemptions + quantity > deal.perCustomerLimit) {
                    throw new BadRequestException(`You can only purchase ${deal.perCustomerLimit} of this deal`);
                }
            }
    ...
    async redeem(redemptionCode: string, user: User) {
        ...
        if (redemption.deal.business.id !== user.id) {
            throw new BadRequestException('You are not authorized to redeem this code');
        }
        ...
        if (redemption.status !== RedemptionStatus.PENDING) {
            throw new BadRequestException(`Cannot redeem code with status ${redemption.status}`);
        }
    ...
    async claimReward(dealId: string, user: User) {
        ...
        try {
            ...
            const balances: ParticipantCampaignBalance[] = await queryRunner.manager.find(ParticipantCampaignBalance, {
                where: [
                    { participant: { id: user.id }, businessCampaign: { business: { id: businessId } } },
                    { participant: { id: user.id }, campaign: { business: { id: businessId } } }
                ],
                relations: ['businessCampaign', 'campaign', 'participant'],
                lock: { mode: 'pessimistic_write' }
            });

            const totalPoints = balances.reduce((sum, b) => sum + b.campaign_balance, 0);

            if (totalPoints < deal.pointsCost) {
                throw new BadRequestException(`Insufficient points from this business. You have ${totalPoints}, need ${deal.pointsCost}.`);
            }
