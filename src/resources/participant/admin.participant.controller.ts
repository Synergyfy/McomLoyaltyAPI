...
export class AdminParticipantController {
  ...
  async findAll(@Query() paginationDto: PaginationDto) {
    return this.participantService.findAll(paginationDto.page, paginationDto.limit);
  }
  ...
  @ApiResponse({ status: 204, description: 'The participant has been successfully deleted.' })
  @ApiResponse({ status: 404, description: 'Participant not found.' })
  ...
  @ApiResponse({ status: 200, description: 'The participant has been successfully disabled.' })
  @ApiResponse({ status: 404, description: 'Participant not found.' })
  ...
  @ApiOperation({ summary: 'Remove a participant from a campaign (admin only)' })
  @ApiResponse({ status: 204, description: 'The participant has been successfully removed from the campaign.' })
  @ApiResponse({ status: 404, description: 'Participant or campaign not found.' })
  async removeFromCampaign(
    @Param('participantId') participantId: string,
    @Param('campaignId') campaignId: string,
  ) {
    return this.participantService.removeFromCampaign(participantId, campaignId);
  }

  @Get(':id/history')
  @ApiOperation({ summary: 'Get a participant\'s earning and redeeming history (admin only)' })
  @ApiResponse({
    status: 200,
    description: 'A paginated list of a participant\'s earning and redeeming history.',
    type: [PointHistory],
  })
  async getHistory(
    @Param('id') id: string,
