...
export class MembershipGuard implements CanActivate {
    ...
    async canActivate(context: ExecutionContext): Promise<boolean> {
        const skipCheck = this.reflector.getAllAndOverride<boolean>(SKIP_MEMBERSHIP_CHECK_KEY, [
            context.getHandler(),
            context.getClass(),
        ]);
        ...
        const membership = await this.membershipService.findOneByBusinessId(user.id);
